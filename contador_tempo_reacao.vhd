...
-- Contador de tempo de reação (ms) para clock de 2,5 kHz.
-- Inicia ao detectar borda de subida em 'start'.
-- Para quando B=1 ou ao atingir 2000 ms (timeout), indicando 'lento'.
-- Saídas ficam latched até novo reset.
entity contador_tempo_reacao is
  ...
  port (
    ...
    start  : in  std_logic;                       -- sobe para iniciar (ex.: len)
    B      : in  std_logic;                       -- botão (pressão = '1'), assumindo já debounced
    rtempo : out std_logic_vector(11 downto 0);   -- tempo de reação em ms (0..4095)
    lento  : out std_logic;                       -- 1 se excedeu 2 s sem apertar
    done   : out std_logic                        -- 1 quando medir terminou (botão ou timeout)
  );
...
architecture rtl of contador_tempo_reacao is
  ...
  -- 1000 < 2^10 (=1024), 1000+400=1400 < 2^11 (=2048) -> 11 bits bastam.
    ...
    if rst = '1' then
      ...
      if (counting = '1') and (done_r = '0') then
        ...
        if B = '1' then
          counting <= '0';
          done_r   <= '1';
          -- ms_count já contém o tempo até o último tick processado
        else
