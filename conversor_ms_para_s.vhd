...
-- Converte milissegundos (ms) para segundos + milissegundos (s.ms)
-- Ex.: 1734 ms -> s_int=1, s_milli=734
entity conversor_ms_para_s is
  generic (
    MAX_MS : natural := 2000   -- opcional: saturar acima de 2 s (ajuste se quiser)
  );
  port (
    rtempo_ms : in  std_logic_vector(11 downto 0); -- 12 bits (0..4095 ms)
    s_int     : out std_logic_vector(1 downto 0);  -- segundos inteiros (0..3)
...
begin
  ...
  -- Saturação opcional em MAX_MS (evita mostrar >2.000 ms se esse for o requisito)
  ...
  -- Divisão por constante (sintetizável): segundos inteiros e resto em ms
